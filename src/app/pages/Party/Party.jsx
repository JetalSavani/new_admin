...
export default function Party() {
  ...
  const editBtnClick = (row) => {
    ...
    setNewData({
      id: row?._id,
      name: row?.partyTypeName,
      //   date: moment(row?.date).format("YYYY-MM-DD"),
      //   country: row?.country,
    });
  };
  const onDelete = async (Id) => {
    await ApiDelete(`/delete/partyType/${Id}`)
      .then(() => {
        Swal.fire({
  ...
  const onUpdate = async () => {
    let body = {
      id: newData?.id,
      partyTypeName: newData?.name,
      //   date: moment(newData?.date).format("MM-DD-YYYY"),
      //   country: newData?.country,
    };
    console.log("body", body);
    await ApiPut("/update/partyType", body)
      .then((res) => {
  ...
  const handleSubmit = async () => {
    ...
    await ApiPost("/add/partyType", body)
      .then((res) => {
        SuccessToast(res?.data?.message);
        setModal(false);
        fetchData(currentpage, pagesize, searching);
