...
export default function Category() {
  ...
  const onDelete = async (Id) => {
    await ApiDelete(`/category/delete-category?id=${Id}`)
      .then(() => {
        Swal.fire({
  ...
  const onUpdate = async () => {
    ...
    await ApiPut(`/category/update-category?id=${newData?.id}`, body)
      .then((res) => {
  ...
  const handleSubmit = async () => {
    ...
    await ApiPost("/category/add-category", body)
      .then((res) => {
        SuccessToast(res?.data?.message);
        setModal(false);
        getCategory();
  ...
  return (
    <>
      <div className="d-flex justify-content-between mb-4">
        <div className="title">
          <div className="fs-20px fw-bolder">Animal List</div>
          <div>
            <span
              role="button"
